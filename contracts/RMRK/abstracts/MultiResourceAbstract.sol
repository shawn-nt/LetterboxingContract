...
abstract contract MultiResourceAbstract is
    IRMRKMultiResource,
    MultiResourceAbstractBase
{
        ...
        returns (Resource memory)
    {
        ...
        if (resource.id == uint64(0)) revert RMRKNoResourceMatchingId();
        return resource;
    }
    ...
    function _addResourceEntry(
        ...
        if (_resources[id].id != uint64(0)) revert RMRKResourceAlreadyExists();
    ...
    function _removeCustomDataFromResource(uint64 resourceId, uint256 index)
        internal
    {
